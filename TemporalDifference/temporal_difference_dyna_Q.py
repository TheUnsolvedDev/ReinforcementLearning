...
class EnvMaze:
    ...
    def reset(self):
        ...
        self.done = False
        return self.current[0]*self.maze.shape[0] + self.current[1]

    def step(self, action):
        if (self.maze[self.current[0] + self.movements[action][0]][self.current[1] + self.movements[action][1]]) != 1:
            ...
            self.current[1] = np.clip(
                self.current[1] + self.movements[action][1], 1, self.shapes[1]-2);print(self.current,self.movements[action],self.maze[10][7])
        self.maze[self.current[0]][self.current[1]] = 8
        state = self.current[0]*self.maze.shape[0] + self.current[1]
