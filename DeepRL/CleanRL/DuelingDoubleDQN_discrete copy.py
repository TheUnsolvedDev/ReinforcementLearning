...
wandb.init(name='DuelingDouble-DQN_'+config['env_name'],
...
class Agent:
    ...
    def train_step(self, states, actions, rewards, next_states, done):
        with tf.GradientTape() as tape:
            ...
            next_indices = tf.stack([tf.range(
                args.batch_size), tf.argmax(self.model.model(next_states), axis=1, output_type=tf.int32)], axis=1)
            next_q_values = tf.gather_nd(
                self.target_model.model(next_states), indices=next_indices)
    ...
    def train(self, max_episodes=args.max_episodes):
        ...
        for ep in tqdm.tqdm(range(max_episodes)):
            ...
            if self.buffer.size() >= args.batch_size:
                loss = self.replay()
            self.target_update()
            self.target_model.model.save_weights('weights/DuelingDouble_DQN_weights_'+config['env_name']+'.h5')
            wandb.log({'Loss': loss})
