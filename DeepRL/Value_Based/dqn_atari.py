...
def train_env(env, video=True):
    if video:
        env = gym.wrappers.RecordVideo(
            env, video_folder='updates/DQN_atari_'+ENV_NAME, episode_trigger=lambda x: x % 1 == 0)
...
class QModel(tf.keras.Model):
    def __init__(self, name='Model'):
        ...
        self.lambda_layer = tf.keras.layers.Lambda(lambda x: x/255.0)
        self.conv1 = tf.keras.layers.Conv2D(
            filters=6, kernel_size=(3, 3), activation='relu', data_format='channels_first')
        self.conv2 = tf.keras.layers.Conv2D(
            filters=16, kernel_size=(3, 3), activation='relu', data_format='channels_first')
...
class ReplayBuffer:
    def __init__(self, size=BUFFER_SIZE):
        self.capacity = size
        self.storage = {
            'state': np.zeros((self.capacity, *OBSERVATION_SHAPE)).astype(np.uint8),
            'action': np.zeros(self.capacity).astype(np.int32),
            'reward': np.zeros(self.capacity).astype(np.float32),
            'next_state': np.zeros((self.capacity, *OBSERVATION_SHAPE)).astype(np.uint8),
...
class DQNAgent:
    def __init__(self) -> None:
        ...
        self.q_model.summary(expand_nested=True)
    ...
    def load(self, path='weights/'):
        self.q_model.load_weights(path+ENV_NAME+'dqn_atari_q_model.h5')
        self.target_model.load_weights(path+ENV_NAME+'dqn_atari_q_target.h5')

    def save(self, path='weights/'):
        self.q_model.save_weights(path+ENV_NAME+'dqn_atari_q_model.h5')
        self.target_model.save_weights(path+ENV_NAME+'dqn_atari_q_target.h5')
...
def plot_and_save(data, average_most=5):
    ...
    ax.legend()
    plt.savefig('plots/dqn_atari_'+ENV_NAME+'.png')
    plt.close()
